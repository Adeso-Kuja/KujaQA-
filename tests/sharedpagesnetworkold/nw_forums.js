...
export async function the_forumsed ( page ) {
  ...
  await expect(page.getByText('ForumsAdd forumView all forums')).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Filter by Focus Areas ' }).click({ timeout: 5000 });

  await page.getByRole('menuitem', { name: 'Adult Welfare' }).click();
  await page.getByRole('button', { name: 'Adult Welfare Delete ' }).click();
 // await page.getByRole('link', { name: 'Delete' }).click();
  await expect(page.getByText('Sort by:Newest to')).toBeVisible();
  await expect(page.getByLabel('Sort by:Newest to')).toBeVisible();
 


    // Scope to the filter/action bar
     const createButton = page.locator('main')
    .filter({ has: page.getByRole('button', { name: /Filter/ }) })
    .getByRole('button', { name: 'Create a forum' });

    // individualaccount\userprofile.js:123
await page.getByRole('button', { name: 'Create a forum' }).first().click();


  await expect(page.getByText('Forum Title')).toBeVisible();
  ...
  await page.locator('textarea').fill('test forum in automated way');
  ...
  //View Saved Forums
  await expect(page.getByRole('link', { name: 'Forum Topics' })).toBeVisible();
