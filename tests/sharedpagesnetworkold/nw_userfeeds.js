...
export async function userfeeds ( page ) {
 ...
 //await page.locator('.uf-profile-avatar').click();
  ...
  await expect(page.getByText('Your Profile isn\'t complete!')).toBeVisible();
  await expect(page.getByRole('link', { name: 'Completearrow' })).toBeVisible();
  await page.getByRole('link', { name: 'Completearrow' }).click();
