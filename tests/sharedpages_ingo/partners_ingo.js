...
export async function partners_ingo ( page ) {
 ...
 // await expect(page.getByRole('heading', { name: 'Partners', exact: true })).toBeVisible();
  ...
  await page.getByRole('link', { name: 'Add Add a partner' }).click({ timeout: 100000 });
    await page.locator('.o_select_menu_toggler').click({ timeout: 100000 });

  await page.getByRole('textbox', { name: 'Search...' }).click({ timeout: 100000 });
