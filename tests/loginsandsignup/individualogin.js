...
//import {getLatestOTP} from '../lib/gmail-helper';
//import {getLatestOTP} from '../lib/outlook-helper';
import {getLatestOTP} from '../lib/mailpit-helper.js';
import  { testData } from '../lib/variables.js';


export async function individual_login_confirmpage_details ( page ) {

  await page.goto('/');
  await page.getByRole('link', { name: 'Log In' }).click();
  await expect(page.getByRole('link', { name: ' Log in with your email' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Welcome to Kuja' })).toBeVisible();
  await expect(page.getByText('Log in with your social')).toBeVisible();
  await expect(page.getByRole('button', { name: ' Log in with google' })).toBeVisible();
  await expect(page.getByRole('button', { name: ' Log in with linkedin' })).toBeVisible();
  await expect(page.getByRole('button', { name: ' Log in with twitter' })).toBeVisible();
  await expect(page.locator('div:nth-child(6)').first()).toBeVisible();
  await expect(page.getByText('Don\'t have an account?')).toBeVisible();

}

export async function _login_google ( page ) {

  await page.goto('/partners/signin');
  await page.getByRole('button', { name: ' Log in with google' }).click();
  await page.getByRole('textbox', { name: 'Email or phone' }).click();
  await page.getByRole('textbox', { name: 'Email or phone' }).fill(testData.individual_usergEmail);
  await page.getByRole('button', { name: 'Next' }).click();
  await page.goto('/partners/signin');

}

export async function linkedin_login ( page ) {
 await page.goto('/');
  await page.getByRole('link', { name: 'Log In' }).click();
  await page.getByRole('button', { name: ' Log in with linkedin' }).click();
  await page.getByRole('textbox', { name: 'Email or Phone' }).click();
  await page.getByRole('textbox', { name: 'Email or Phone' }).fill(testData.individual_usergEmail);
...
export async function twitter_login ( page ) {
     await page.getByRole('link', { name: 'Log In' }).click();
  await expect(page.getByRole('button', { name: ' Log in with twitter' })).toBeVisible();
  await page.getByRole('button', { name: ' Log in with twitter' }).click();
  await page.getByRole('textbox', { name: 'Phone, email, or username' }).fill(testData.individual_usergEmail);
  await page.getByRole('button', { name: 'Next' }).click();
  await page.goto('/partners/signin');
}
...
export async function loginwith_email_login ( page ) {
  await page.goto('/partners/signin');
  ...
  await page.getByRole('textbox', { name: 'Enter Email Address' }).fill(testData.individualstaging);
// ingo_userEmail
  // ... (Step 1: Submit Email) ...
